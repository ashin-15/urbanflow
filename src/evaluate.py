...
def evaluate_pipeline(test_path="data/processed/test.csv", models_dir="models", reports_dir="reports"):
    ...
    # Sample up to 2000 points for clear visual scatter plot
    sample_size = min(2000, len(y_test))
    sample_indices = np.random.choice(len(y_test), sample_size, replace=False)
    y_test_sample = y_test.iloc[sample_indices]
    
    plt.figure(figsize=(10, 8))
    sns.scatterplot(x=y_test_sample, y=predictions["ensemble"][sample_indices], alpha=0.4, color="#1f77b4", label="Ensemble", s=15)
    ...
    # By hour flag
    df_test_trans['ens_preds'] = ens_preds
    df_test_trans['residual'] = y_test.values - ens_preds
    
    segment_analysis += "\n### By Time Period (Hour Flag)\n\n"
    segment_analysis += "| Time Period | Count | Mean Actual | Mean Predicted | Mean Error | R² |\n"
    segment_analysis += "|---|---|---|---|---|---|\n"
    
    for hf in ['early_morning', 'morning_rush', 'midday', 'evening_rush', 'night']:
        mask = df_test_trans['hour_flag'] == hf
        if mask.sum() > 10:
            y_seg = y_test[mask]
            p_seg = ens_preds[mask]
            seg_r2 = r2_score(y_seg, p_seg) if len(y_seg) > 1 else float('nan')
            segment_analysis += (
                f"| {hf.replace('_', ' ').title()} | {mask.sum()} | "
                f"{y_seg.mean():.0f} | {p_seg.mean():.0f} | "
                f"{(y_seg.values - p_seg).mean():.0f} | {seg_r2:.4f} |\n"
            )
    
    # By road type
    if 'road_type' in df_test_trans.columns:
        segment_analysis += "\n### By Road Type\n\n"
        segment_analysis += "| Road Type | Count | Mean Actual | Mean Predicted | Mean Error | R² |\n"
        segment_analysis += "|---|---|---|---|---|---|\n"
        
        for rt in df_test_trans['road_type'].unique():
            mask = df_test_trans['road_type'] == rt
            if mask.sum() > 10:
                y_seg = y_test[mask]
                p_seg = ens_preds[mask]
                seg_r2 = r2_score(y_seg, p_seg) if len(y_seg) > 1 else float('nan')
    ...
    # 6. Generate Evaluation Report Markdown
    report_path = os.path.join(reports_dir, "evaluation_report.md")
    
    ens_weights_str = f"RF: {ensemble.rf_weight:.0%}, XGB: {ensemble.xgb_weight:.0%}, LGBM: {ensemble.lgbm_weight:.0%}"
    
    md_content = f"""# Model Evaluation Report

This report presents the final evaluation metrics of the Traffic Demand Prediction System on the held-out test dataset ({len(y_test):,} records, chronological split).
...
> Ensemble weights: {ens_weights_str}

## 2. Key Findings & Discussion

1. **Ensemble Performance:** The Weighted Ensemble achieved an R² score of **{metrics['ensemble']['r2']:.4f}** on the held-out test set.
2. **Gradient Boosting vs. Bagging:** XGBoost (R²: {metrics['xgb']['r2']:.4f}) and LightGBM (R²: {metrics['lgbm']['r2']:.4f}) vs Random Forest (R²: {metrics['rf']['r2']:.4f}).
